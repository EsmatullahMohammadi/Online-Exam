...
const Condidate = () => {
  ...
  const deleteCandidate = async (candidateID) => {
    ...
    try {
      const res = await axios.delete(`${SUPER_DOMAIN}/candidates/${candidateID}`);
  ...
  const handleTestChange = (e) => {
    ...
    if (testId) {
      // Navigate to filter-by-test with query param
        ...
        <table className="w-full table-auto">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan="5" className="text-center py-3 text-red-500">{error}</td></tr>
                ...
                <tr key={cand._id} className="border-b">
                  ...
                  <td className="py-3 px-4">
                    <span className={`inline-flex rounded-full py-1 px-3 text-sm font-medium {
                      cand.status === 'Pending' ? 'bg-warning text-warning' :
                      cand.status === 'Passed' ? 'bg-success text-success' : 'bg-danger text-danger'
                    }`}>{cand.status}</span>
                  </td>
                  <td className="py-3 px-4">
                    <div className="flex space-x-3">
                      <MdVisibility
                        ...
                        onClick={() => { setSelectedCandidate(cand); setIsOpenModel(true); }}
                      />
                      <MdDelete
                        className="text-2xl text-red-500 cursor-pointer"
                        onClick={() => deleteCandidate(cand._id)}
                      />
                      <Link to="/admin/condidate/edit-candidate" state={{ candidate: cand }}>
              ...
              <tr><td colSpan="5" className="text-center py-3">No candidates found.</td></tr>
            )}
          </tbody>
        </table>
        <Pagination
